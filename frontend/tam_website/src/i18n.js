import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';

const resources = {
  en: {
    translation: {
      // Header Navigation
      home: 'Home',
      news: 'News',
      shop: 'Shop',
      contact: 'Contact',
      about: 'About',
      search: 'Search...',
      selectLanguage: 'Select your language',
      persian: 'فارسی',
      english: 'English',
      // Latest News Section
      latestNews: 'Latest News',
      latestVideos: 'Latest Videos',
      viewAll: 'VIEW ALL',
      // Team Section
      tamsTeam: "TAM'S TEAM",
      football: "FOOTBALL",
      // Shop Section
      tamsShop: "TAM'S SHOP",
      comingSoon: "COMING SOON",
      shopUnderConstruction: "Our shop is under construction. Stay tuned!",
      // Shop Page
      shopPageComingSoon: "Coming Soon",
      shopPagePreparing: "We're preparing something special for you",
      shopPageTitle: "TamSport Shop",
      shopPageDescription: "Our online store is currently under development. We're working hard to bring you the best selection of team merchandise and exclusive items.",
      shopPageBackToHome: "Back to Home",
      // About Page
      aboutTeamName: "Tam Cultural and Sports Club",
      aboutTeamDescription: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Egestas purus viverra accumsan in nisl nisi. Arcu cursus vitae congue mauris rhoncus aenean vel elit scelerisque. In egestas erat imperdiet sed euismod nisi porta lorem mollis. Morbi tristique senectus et netus. Mattis pellentesque id nibh tortor id aliquet lectus proin. Sapien faucibus et molestie ac feugiat sed lectus vestibulum. Ullamcorper velit sed ullamcorper morbi tincidunt ornare massa eget. Dictum varius duis at consectetur lorem. Nisi vitae suscipit tellus mauris a diam maecenas sed enim. Velit ut tortor pretium viverra suspendisse potenti nullam. Et molestie ac feugiat sed lectus. Non nisi est sit amet facilisis magna. Dignissim diam quis enim lobortis...",
      aboutReadMore: "Read More",
      aboutShowLess: "Show Less",
      aboutImportantHonors: "Important Honors",
      aboutPremierLeagueChampionship: "Premier League Championship 2008/09",
      // Player Section
      player: "PLAYER",
      forward: "Forward",
      // Contact Section
      whatsapp: "FC TAM WHATSAPP",
      instagram: "FC TAM INSTAGRAM",
      discord: "FC TAM DISCORD",
      twitter: "FC TAM TWITTER",
      youtube: "FC TAM YOUTUBE",
      facebook: "FC TAM FACEBOOK",
      // Footer Section
      footerHome: "Home",
      footerNews: "News",
      footerShop: "Shop",
      footerAbout: "About",
      footerLatestNews: "Latest News",
      footerTamsTeam: "Tam's Team",
      footerTamsShop: "Tam's Shop",
      footerPlayers: "Players",
      footerAllContent: "All Content",
      footerArticleContent: "Article Content",
      footerVideoContent: "Video Content",
      footerSlideshowContent: "Slideshow Content",
      footerComingSoon: "Coming Soon",
      footerTeamName: "Team Name",
      footerTeamLogo: "Team Logo",
      footerTeamDescriptions: "Team Descriptions",
      footerTeamHonors: "Team Honors",
      // News Page
      newsAll: "All",
      newsArticles: "Articles",
      newsVideos: "Videos",
      newsSlideshows: "Slideshows",
      newsLoadMore: "Load More",
      // Error Page
      errorPageNotFound: "Page Not Found",
      errorPageDescription: "The page you are looking for doesn't exist or has been moved.",
      errorBackToHome: "Back to Home",
      // Login Page
      loginWelcomeTitle: "Welcome To Website",
      loginWelcomeDescription: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua Egestas purus viverra accumsan in nisl nisi Arcu cursus vitae congue mauris rhoncus aenean vel elit scelerisque ...",
      userLogin: "User Login",
      userRegister: "User Register",
      phoneNumber: "Phone Number",
      password: "Password",
      confirmPassword: "Confirm Password",
      rememberMe: "Remember me",
      forgotPassword: "Forgot Password?",
      login: "LOGIN",
      register: "Register",
      or: "OR",
      dontHaveAccount: "Don't have an account?",
      alreadyHaveAccount: "Already have an account?",
      invalidCredentials: "Phone number or password is incorrect",
      invalidPhoneNumber: "Your Phone Number is Invalid",
      passwordsDoNotMatch: "Passwords do not match",
      passwordLengthError: "Password must be at least 8 characters",
      passwordContentError: "Use letters and numbers",
      otpVerification: "OTP Verification",
      otpEnterCode: "Enter the code sent to your phone",
      invalidOtpCode: "Invalid OTP code",
      resendCode: "Resend Code",
      verify: "Verify",
      loginSuccessTitle: "Login Successful",
      loginSuccessContent: "You have successfully logged in!",
      // Modal
      modalAccept: "Accept",
      modalReject: "Reject",
      // Welcome Modal
      welcomeModalTitle: "Welcome to TamSport",
      welcomeModalContent: "We are delighted to have you here. Explore our website to discover the latest news, team information, and exclusive content about TamSport.",
      // Dashboard
      dashboardWelcomeBack: "Welcome Back!",
      dashboardLogout: "Logout",
      dashboardProfile: "Profile",
      dashboardOrders: "Orders",
      dashboardFavorites: "Favorites",
      dashboardSettings: "Settings",
      dashboardProfileInfo: "Profile Information",
      dashboardEmail: "Email",
      dashboardEnterEmail: "Enter your email",
      dashboardUpdateEmail: "Update Email",
      dashboardChangePassword: "Change Password",
      dashboardCurrentPassword: "Current Password",
      dashboardNewPassword: "New Password",
      dashboardConfirmNewPassword: "Confirm New Password",
      dashboardYourOrders: "Your Orders",
      dashboardNoOrders: "No orders found.",
      dashboardYourFavorites: "Your Favorites",
      dashboardNoFavorites: "No favorites found.",
      dashboardAccountSettings: "Account Settings",
      dashboardEmailNotifications: "Email Notifications",
      dashboardEmailNotificationsDesc: "Receive updates about your orders",
      dashboardSmsNotifications: "SMS Notifications",
      dashboardSmsNotificationsDesc: "Receive updates via SMS",
      dashboardImageSizeError: "Image size should be less than 5MB",
      dashboardImageUpdateSuccess: "Profile picture updated successfully",
      dashboardPasswordChangeSuccess: "Password changed successfully",
      dashboardEmailUpdateSuccess: "Email updated successfully",
      // Show/Hide Buttons
      dashboardShowEmailForm: "Change Email",
      dashboardHideEmailForm: "Hide Email Form",
      dashboardShowPasswordForm: "Change Password",
      dashboardHidePasswordForm: "Hide Password Form",
      // Admin Panel
      adminWelcome: "Welcome to Admin Panel",
      adminLogout: "Logout",
      adminPanel: "Admin Panel",
      adminDashboard: "Dashboard",
      adminUsers: "Users",
      adminNews: "News",
      adminShop: "Shop",
      adminSettings: "Settings",
      adminDashboardOverview: "Dashboard Overview",
      adminTotalUsers: "Total Users",
      adminTotalNews: "Total News",
      adminTotalProducts: "Total Products",
      adminUsersManagement: "Users Management",
      adminNewsManagement: "News Management",
      adminShopManagement: "Shop Management",
      adminAddUser: "Add User",
      adminAddNews: "Add News",
      adminAddProduct: "Add Product",
      adminPhoneNumber: "Phone Number",
      adminActions: "Actions",
      adminEdit: "Edit",
      adminDelete: "Delete",
      adminTitle: "Title",
      adminCategory: "Category",
      adminType: "Type",
      adminViews: "Views",
      adminNoProducts: "No products available",
      adminGeneralSettings: "General Settings",
      adminMaintenanceMode: "Maintenance Mode",
      adminEnableComments: "Enable Comments",
      // User Management
      adminUserForm: "User Form",
      adminAddNewUser: "Add New User",
      adminEditUser: "Edit User",
      adminFirstName: "First Name",
      adminLastName: "Last Name",
      adminIsAuthor: "Is Author",
      adminIsSuperuser: "Is Superuser",
      adminIsSeller: "Is Seller",
      adminIsActive: "Is Active",
      adminLastLogin: "Last Login",
      adminCreatedDate: "Created Date",
      adminSaveUser: "Save User",
      adminCancel: "Cancel",
      adminUserSavedSuccess: "User saved successfully",
      adminUserSavedError: "Error saving user",
      adminRequiredField: "This field is required",
      adminInvalidPhoneNumber: "Invalid phone number format",
      adminBackToUsers: "Back to Users",
      // Status
      adminActive: "Active",
      adminInactive: "Inactive",
      adminToggle: "Toggle Status",
      adminStatus: "Status",
      adminClickToToggle: "Click to toggle status",
      adminRole: "Role",
      // Roles
      roleSuperAdmin: "Super Admin",
      roleAdmin: "Admin",
      roleAuthor: "Author",
      roleSeller: "Seller",
      roleUser: "User",
      roleEditor: "Editor",
      // Access Control
      adminAccessDenied: "Access Denied",
      adminInsufficientPermissions: "You don't have permission to access this section",
      adminRoleManagement: "Role Management",
      adminAssignRole: "Assign Role",
      adminChangeRole: "Change Role",
      // Article Management
      articleAddNew: "Add New Article",
      articleEdit: "Edit Article",
      articleAddNewDesc: "Create a new article with bilingual content",
      articleEditDesc: "Edit existing article content and settings",
      articleAuthor: "Author",
      articleSelectAuthor: "Select Author",
      articleCategory: "Category",
      articleTitle: "Title",
      articleTitlePlaceholder: "Enter article title",
      articleBody: "Content",
      articleBodyPlaceholder: "Enter article content",
      articleStatus: "Status",
      articleType: "Type",
      articleSlug: "Slug",
      articleSlugPlaceholder: "article-url-slug",
      articleVideoUrl: "Video URL",
      articleVideoUrlPlaceholder: "https://example.com/video.mp4",
      articleHits: "Views",
      articleLikes: "Likes",
      articleSave: "Save Article",
      articleCancel: "Cancel",
      articleSavedSuccess: "Article saved successfully",
      articleSavedError: "Error saving article",
      articleRequiredField: "This field is required",
      articleTitleRequired: "At least one title (Persian or English) is required",
      articleBodyRequired: "At least one content (Persian or English) is required",
      articleVideoUrlRequired: "Video URL is required for video articles",
      articleDraft: "Draft",
      articleAccepted: "Accepted",
      articleRejected: "Rejected",
      articleSlideshow: "Slideshow",
      articleVideo: "Video",
      articleText: "Text",
    }
  },
  fa: {
    translation: {
      // Header Navigation
      home: 'خانه',
      news: 'اخبار',
      shop: 'فروشگاه',
      contact: 'ارتباط با ما',
      about: 'درباره ما',
      search: 'جستجو...',
      selectLanguage: 'زبان خود را انتخاب کنید',
      persian: 'فارسی',
      english: 'English',
      // Latest News Section
      latestNews: 'جدیدترین اخبار',
      latestVideos: 'جدیدترین ویدیوها',
      viewAll: 'مشاهده همه',
      // Team Section
      tamsTeam: "تیم‌های تام",
      football: "فوتبال",
      // Shop Section
      tamsShop: "فروشگاه تام",
      comingSoon: "به زودی",
      shopUnderConstruction: "فروشگاه ما در حال ساخت است. با ما همراه باشید!",
      // Shop Page
      shopPageComingSoon: "به زودی",
      shopPagePreparing: "ما در حال آماده‌سازی چیزی خاص برای شما هستیم",
      shopPageTitle: "فروشگاه تام اسپرت",
      shopPageDescription: "فروشگاه آنلاین ما در حال توسعه است. ما سخت در تلاش هستیم تا بهترین انتخاب از محصولات تیم و اقلام انحصاری را برای شما فراهم کنیم.",
      shopPageBackToHome: "بازگشت به خانه",
      // About Page
      aboutTeamName: "باشگاه فرهنگی ورزشی تام",
      aboutTeamDescription: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است. چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با هدف بهبود ابزارهای کاربردی می باشد. کتابهای زیادی در شصت و سه درصد گذشته، حال و آینده شناخت فراوان جامعه و متخصصان را می طلبد تا با نرم افزارها شناخت بیشتری را برای طراحان رایانه ای علی الخصوص طراحان خلاقی و فرهنگ پیشرو در زبان فارسی ایجاد کرد. در این صورت می توان امید داشت که تمام و دشواری موجود در ارائه راهکارها و شرایط سخت تایپ به پایان رسد و زمان مورد نیاز شامل حروفچینی دستاوردهای اصلی و جوابگوی سوالات پیوسته اهل دنیای موجود طراحی اساسا مورد استفاده قرار گیرد...",
      aboutReadMore: "ادامه مطلب",
      aboutShowLess: "کمتر",
      aboutImportantHonors: "افتخارات مهم",
      aboutPremierLeagueChampionship: "قهرمانی لیگ برتر 2008/09",
      // Player Section
      player: "بازیکن",
      forward: "مهاجم",
      // Contact Section
      whatsapp: "واتساپ تام",
      instagram: "اینستاگرام تام",
      discord: "دیسکورد تام",
      twitter: "توییتر تام",
      youtube: "یوتیوب تام",
      facebook: "فیسبوک تام",
      // Footer Section
      footerHome: "خانه",
      footerNews: "اخبار",
      footerShop: "فروشگاه",
      footerAbout: "درباره ما",
      footerLatestNews: "جدیدترین اخبار",
      footerTamsTeam: "تیم تام",
      footerTamsShop: "فروشگاه تام",
      footerPlayers: "بازیکنان",
      footerAllContent: "همه محتوا",
      footerArticleContent: "محتوا متنی",
      footerVideoContent: "محتوا ویدیویی",
      footerSlideshowContent: "محتوا اسلایدشو",
      footerComingSoon: "به زودی",
      footerTeamName: "نام تیم",
      footerTeamLogo: "لوگوی تیم",
      footerTeamDescriptions: "توضیحات تیم",
      footerTeamHonors: "افتخارات تیم",
      // News Page
      newsAll: "همه",
      newsArticles: "مقالات",
      newsVideos: "ویدیوها",
      newsSlideshows: "اسلایدشوها",
      newsLoadMore: "بارگذاری بیشتر",
      // Error Page
      errorPageNotFound: "صفحه یافت نشد",
      errorPageDescription: "صفحه‌ای که به دنبال آن هستید وجود ندارد یا منتقل شده است.",
      errorBackToHome: "بازگشت به خانه",
      // Login Page
      loginWelcomeTitle: "به وبسایت خوش آمدید",
      loginWelcomeDescription: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با استفاده از طراحان گرافیک است. چاپگرها و متون بلکه روزنامه و مجله در ستون و سطرآنچنان که لازم است و برای شرایط فعلی تکنولوژی مورد نیاز و کاربردهای متنوع با هدف بهبود ابزارهای کاربردی می باشد...",
      userLogin: "ورود کاربر",
      userRegister: "ثبت نام کاربر",
      phoneNumber: "شماره موبایل",
      password: "رمز عبور",
      confirmPassword: "تکرار رمز عبور",
      rememberMe: "مرا به خاطر بسپار",
      forgotPassword: "رمز عبور را فراموش کرده‌اید؟",
      login: "ورود",
      register: "ثبت نام",
      or: "یا",
      dontHaveAccount: "حساب کاربری ندارید؟",
      alreadyHaveAccount: "قبلاً ثبت نام کرده‌اید؟",
      invalidCredentials: "شماره موبایل یا رمز عبور اشتباه است",
      invalidPhoneNumber: "شماره موبایل نامعتبر است",
      passwordsDoNotMatch: "رمزهای عبور مطابقت ندارند",
      passwordLengthError: "رمز عبور باید حداقل 8 کاراکتر باشد",
      passwordContentError: "از حروف و اعداد استفاده کنید",
      otpVerification: "تایید کد یکبار مصرف",
      otpEnterCode: "کد ارسال شده به موبایل خود را وارد کنید",
      invalidOtpCode: "کد یکبار مصرف نامعتبر است",
      resendCode: "ارسال مجدد کد",
      verify: "تایید",
      loginSuccessTitle: "ورود موفق",
      loginSuccessContent: "شما با موفقیت وارد شدید!",
      // Modal
      modalAccept: "تایید",
      modalReject: "رد",
      // Welcome Modal
      welcomeModalTitle: "به تام اسپرت خوش آمدید",
      welcomeModalContent: "از حضور شما در وبسایت ما خوشحالیم. برای مشاهده آخرین اخبار، اطلاعات تیم و محتوای اختصاصی تام اسپرت، وبسایت ما را کاوش کنید.",
      // Dashboard
      dashboardWelcomeBack: "خوش آمدید!",
      dashboardLogout: "خروج",
      dashboardProfile: "پروفایل",
      dashboardOrders: "سفارشات",
      dashboardFavorites: "علاقه‌مندی‌ها",
      dashboardSettings: "تنظیمات",
      dashboardProfileInfo: "اطلاعات پروفایل",
      dashboardEmail: "ایمیل",
      dashboardEnterEmail: "ایمیل خود را وارد کنید",
      dashboardUpdateEmail: "بروزرسانی ایمیل",
      dashboardChangePassword: "تغییر رمز عبور",
      dashboardCurrentPassword: "رمز عبور فعلی",
      dashboardNewPassword: "رمز عبور جدید",
      dashboardConfirmNewPassword: "تکرار رمز عبور جدید",
      dashboardYourOrders: "سفارشات شما",
      dashboardNoOrders: "سفارشی یافت نشد.",
      dashboardYourFavorites: "علاقه‌مندی‌های شما",
      dashboardNoFavorites: "مورد علاقه‌ای یافت نشد.",
      dashboardAccountSettings: "تنظیمات حساب کاربری",
      dashboardEmailNotifications: "اعلان‌های ایمیلی",
      dashboardEmailNotificationsDesc: "دریافت به‌روزرسانی‌ها درباره سفارشات",
      dashboardSmsNotifications: "اعلان‌های پیامکی",
      dashboardSmsNotificationsDesc: "دریافت به‌روزرسانی‌ها از طریق پیامک",
      dashboardImageSizeError: "حجم تصویر باید کمتر از 5 مگابایت باشد",
      dashboardImageUpdateSuccess: "تصویر پروفایل با موفقیت به‌روز شد",
      dashboardPasswordChangeSuccess: "رمز عبور با موفقیت تغییر کرد",
      dashboardEmailUpdateSuccess: "ایمیل با موفقیت به‌روز شد",
      // Show/Hide Buttons
      dashboardShowEmailForm: "تغییر ایمیل",
      dashboardHideEmailForm: "مخفی کردن فرم ایمیل",
      dashboardShowPasswordForm: "تغییر رمز عبور",
      dashboardHidePasswordForm: "مخفی کردن فرم رمز عبور",
      // Admin Panel
      adminWelcome: "به پنل مدیریت خوش آمدید",
      adminLogout: "خروج",
      adminPanel: "پنل مدیریت",
      adminDashboard: "داشبورد",
      adminUsers: "کاربران",
      adminNews: "اخبار",
      adminShop: "فروشگاه",
      adminSettings: "تنظیمات",
      adminDashboardOverview: "نمای کلی داشبورد",
      adminTotalUsers: "تعداد کل کاربران",
      adminTotalNews: "تعداد کل اخبار",
      adminTotalProducts: "تعداد کل محصولات",
      adminUsersManagement: "مدیریت کاربران",
      adminNewsManagement: "مدیریت اخبار",
      adminShopManagement: "مدیریت فروشگاه",
      adminAddUser: "افزودن کاربر",
      adminAddNews: "افزودن خبر",
      adminAddProduct: "افزودن محصول",
      adminPhoneNumber: "شماره تماس",
      adminActions: "عملیات",
      adminEdit: "ویرایش",
      adminDelete: "حذف",
      adminTitle: "عنوان",
      adminCategory: "دسته‌بندی",
      adminType: "نوع",
      adminViews: "بازدیدها",
      adminNoProducts: "هیچ محصولی موجود نیست",
      adminGeneralSettings: "تنظیمات عمومی",
      adminMaintenanceMode: "حالت تعمیر و نگهداری",
      adminEnableComments: "فعال‌سازی نظرات",
      // User Management
      adminUserForm: "فرم کاربر",
      adminAddNewUser: "افزودن کاربر جدید",
      adminEditUser: "ویرایش کاربر",
      adminFirstName: "نام",
      adminLastName: "نام خانوادگی",
      adminIsAuthor: "نویسنده",
      adminIsSuperuser: "مدیر سرور",
      adminIsSeller: "فروشنده",
      adminIsActive: "فعال",
      adminLastLogin: "آخرین ورود",
      adminCreatedDate: "تاریخ ایجاد",
      adminSaveUser: "ذخیره کاربر",
      adminCancel: "لغو",
      adminUserSavedSuccess: "کاربر با موفقیت ذخیره شد",
      adminUserSavedError: "خطا در ذخیره کاربر",
      adminRequiredField: "این فیلد اجباری است",
      adminInvalidPhoneNumber: "فرمت شماره موبایل نامعتبر است",
      adminBackToUsers: "بازگشت به کاربران",
      // Status
      adminActive: "فعال",
      adminInactive: "غیرفعال",
      adminToggle: "تغییر وضعیت",
      adminStatus: "وضعیت",
      adminClickToToggle: "برای تغییر وضعیت کلیک کنید",
      adminRole: "نقش",
      // Roles
      roleSuperAdmin: "مدیر سرور",
      roleAdmin: "مدیر",
      roleAuthor: "نویسنده",
      roleSeller: "فروشنده",
      roleUser: "کاربر",
      roleEditor: "ویرایشگر",
      // Access Control
      adminAccessDenied: "دسترسی ممنوع",
      adminInsufficientPermissions: "شما دسترسی به این بخش را ندارید",
      adminRoleManagement: "مدیریت نقش",
      adminAssignRole: "نقش اختصاصی",
      adminChangeRole: "تغییر نقش",
      // Article Management
      articleAddNew: "افزودن مقاله جدید",
      articleEdit: "ویرایش مقاله",
      articleAddNewDesc: "یک مقاله جدید با محتوای دوزبانه ایجاد کنید",
      articleEditDesc: "محتوای مقاله و تنظیمات ویرایش شده",
      articleAuthor: "نویسنده",
      articleSelectAuthor: "نویسنده مقاله را انتخاب کنید",
      articleCategory: "دسته بندی",
      articleTitle: "عنوان",
      articleTitlePlaceholder: "عنوان مقاله را وارد کنید",
      articleBody: "محتوا",
      articleBodyPlaceholder: "محتوای مقاله را وارد کنید",
      articleStatus: "وضعیت",
      articleType: "نوع",
      articleSlug: "Slug",
      articleSlugPlaceholder: "slug-مقاله-url",
      articleVideoUrl: "آدرس ویدیو",
      articleVideoUrlPlaceholder: "https://example.com/video.mp4",
      articleHits: "بازدیدها",
      articleLikes: "پسندها",
      articleSave: "ذخیره مقاله",
      articleCancel: "لغو",
      articleSavedSuccess: "مقاله با موفقیت ذخیره شد",
      articleSavedError: "خطا در ذخیره مقاله",
      articleRequiredField: "این فیلد اجباری است",
      articleTitleRequired: "حداقل یک عنوان (فارسی یا انگلیسی) اجباری است",
      articleBodyRequired: "حداقل یک محتوا (فارسی یا انگلیسی) اجباری است",
      articleVideoUrlRequired: "آدرس ویدیو برای مقاله ویدیویی اجباری است",
      articleDraft: "پیش نویس",
      articleAccepted: "تایید شده",
      articleRejected: "رد شده",
      articleSlideshow: "اسلایدشو",
      articleVideo: "ویدیو",
      articleText: "متن",
    }
  }
};

i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: localStorage.getItem('language') || 'en',
    fallbackLng: 'en',
    interpolation: {
      escapeValue: false
    }
  });

export default i18n; 