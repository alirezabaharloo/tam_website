...
const NewsBox = ({ title, image, type, date }) => {
  const isVideo = type === 'video';
  const isSlideshow = type === 'slideshow';

  return (
    <div className="relative">
      <div className={`relative overflow-hidden rounded-2xl ${isVideo ? 'bg-black/50' : ''}`}>
        <img src={image} alt={title} className="w-full h-[300px] object-cover" />
        {isVideo && (
          <div className="absolute inset-0 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-9 h-9 text-quinary-tint-800">
      ...
      <div className="mt-4">
        <div className="flex items-center">
          ...
          <h3 className="text-[20px] font-inter text-secondary">{title}</h3>
        </div>
        <p className="text-[16px] text-quinary-tint-800 mt-2">{date}</p>
      </div>
    </div>
  );
};
